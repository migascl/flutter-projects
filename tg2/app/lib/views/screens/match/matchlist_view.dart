  ...
  Widget build(BuildContext context) {
    ...
    return Consumer2<ClubProvider, MatchProvider>(builder: (context, clubProvider, matchProvider, child) {
      if(matchProvider.state == ProviderState.ready && clubProvider.state == ProviderState.ready) {
        List<Widget> tabs = List.from(matchProvider.getMatchweeks().map((element) => Tab(text: "Jornada $element")));
        return DefaultTabController(
            ...
            body: TabBarView(
                children: List.from(matchProvider.getMatchweeks().map((element) {
                  List<Match> matchweeks = matchProvider.items.values.where((match) => match.matchweek == element).toList();
      ...
      return Scaffold(appBar: AppBar(title: Text("Clubes")),
      );
    },
    );
  }
